...
</style></head><body><div class="wrap"><h1>暖言聊天自动回复助手</h1><p class="sub">v__VERSION__ · QQ / 微信按选择运行 · 指定联系人精确锁定 · Ctrl+Alt+Q 紧急停止</p><div class="grid"><section class="card">
...
<div class="notice" id="platformNotice">运行时请保持电脑版 QQ 登录，并让锁定联系人的聊天窗口处于打开状态。回复内容由在线模型生成；API Key 只保存在当前 Windows 用户账户中。</div></section><section class="card"><div class="status"><b><span class="dot" id="dot"></span><span id="runLabel">未运行</span></b><span id="status">未运行</span></div><div class="notice">关闭此窗口只会隐藏到右下角托盘，自动回复会继续运行。只有点击“彻底退出程序”才会关闭后台。</div><div class="log" id="logs">暂无日志</div><div class="buttons"><button class="stop" onclick="exitApp()">彻底退出程序</button></div></section></div></div><div class="toast" id="toast"></div>
<script>
